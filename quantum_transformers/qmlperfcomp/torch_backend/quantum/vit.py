...
def VisionTransformer(data_type, img_size, num_channels, num_classes, patch_size, hidden_size, num_heads, num_transformer_blocks, mlp_hidden_size,##  device, 
    ...
    class MultiheadSelfAttention(nn.Module):
        def __init__(self, embed_dim, num_heads, dropout=0.0):
            ...
            self.q_proj = QuantumLayer(num_qubits=embed_dim) ##device=device) #qdevice=qdevice)
            self.k_proj = QuantumLayer(num_qubits=embed_dim) ##device=device) #qdevice=qdevice)
            self.v_proj = QuantumLayer(num_qubits=embed_dim) ##device=device) #qdevice=qdevice)
            self.dropout = nn.Dropout(dropout)
            self.o_proj = QuantumLayer(num_qubits=embed_dim) ##device=device) #qdevice=qdevice)
    ...
    class FeedForward(nn.Module):
        def __init__(self, hidden_size, mlp_hidden_size, dropout=0.0):
            ...
            self.vqc = QuantumLayer(mlp_hidden_size) ##device=device) #qdevice=qdevice)
            self.dropout = nn.Dropout(dropout)
            self.gelu = nn.GELU()
    ...
    class TransformerBlock(nn.Module):
        ...
        def forward(self, x):
            ...
            attn_output, attn_map = self.attn(attn_output)
    ...
    class VisionTransformer(nn.Module):
        def __init__(self, data_type, img_size, num_channels, num_classes, patch_size, hidden_size, num_heads, num_transformer_blocks, mlp_hidden_size,
            ...
            self.prueba = [f"Q, data_type={data_type}, patch_size={patch_size}, hidden_size={hidden_size}, num_heads={num_heads}, num_transformer_blocks={num_transformer_blocks}, mlp_hidden_size={mlp_hidden_size}"]
        ...
        def forward(self, x):
            ...
            for i, transformer_block in enumerate(self.transformer_blocks):      ###
                x, attn_map = transformer_block(x)                               ###
            ...
            x.attentions = attn_maps                                             ###
            
            return x

    return VisionTransformer(data_type, img_size, num_channels, num_classes, patch_size, hidden_size, num_heads, num_transformer_blocks, mlp_hidden_size,
