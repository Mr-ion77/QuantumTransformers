...
class QuantumLayer(torch.nn.Module):
    def __init__(self, num_qubits, num_qlayers=1): #qdevice='default.qubit', diff_method="best"):
        ...
        dev = qml.device('default.qubit.torch', wires=num_qubits, torch_device="cuda:0")
        @qml.qnode(dev, interface='torch', diff_method="backprop")
        def circuit(inputs, weights):
                qml.templates.AngleEmbedding(inputs, wires=range(num_qubits))           # DIGITAL ENCODING OF THE PIXELS
                qml.templates.BasicEntanglerLayers(weights, wires=range(num_qubits))    # Trainable rotations around X,Y,Z axes +
                                                                                        # entanglement via CNOT gate in cascade through indexes = wires.
                return [qml.expval(qml.PauliZ(wires=i)) for i in range(num_qubits)]     # Beware that range of output is between eigenvalues -1 and 1, 
                                                                                        # not conventional 0 and 1.
